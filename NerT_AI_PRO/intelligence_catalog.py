...
def compute_prediction_level(
    ...
    *,
    ...
    """Calcula nivel L0-L4 a partir de métricas live y umbrales."""
    ...
    if not data_ok or not calibrated:
        level = "L0"
        meta = PREDICTION_LEVELS[0]
    elif abs(combined) < hold_band:
        level = "L1"
        meta = PREDICTION_LEVELS[1]
    elif combined >= buy_th and egm > -0.01:
        level = "L3"
        meta = PREDICTION_LEVELS[3]
        if abs(combined) >= buy_th * 1.2:
            level = "L4"
            meta = PREDICTION_LEVELS[4]
    elif combined <= sell_th and egm < 0.01:
        level = "L3"
        meta = PREDICTION_LEVELS[3]
        if abs(combined) >= abs(sell_th) * 1.2:
            level = "L4"
            meta = PREDICTION_LEVELS[4]
    else:
        level = "L2"
        meta = PREDICTION_LEVELS[2]
    ...
    return {
        ...
        "thresholds": {
            "combined_buy": buy_th,
            "combined_sell": sell_th,
            "combined_hold_band": hold_band,
        },
