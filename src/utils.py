...
def _results_now_iso() -> str:
    return datetime.now(timezone.utc).isoformat()


def _results_filepath(log_dir: str) -> str:
    return os.path.join(os.path.abspath(log_dir), "results.json")


def _results_backup_path(filepath: str) -> str:
    return f"{filepath}.bak"


def _repair_truncated_results_json(text: str) -> Optional[Dict[str, Any]]:
    text = text.strip()
    if not text:
        return None
    for cut in range(len(text), max(0, len(text) - 5000), -1):
        chunk = text[:cut].rstrip().rstrip(",")
        if not chunk:
            continue
        open_b = chunk.count("{") - chunk.count("}")
        open_a = chunk.count("[") - chunk.count("]")
        suffix = ("]" * max(0, open_a)) + ("}" * max(0, open_b))
        try:
            data = json.loads(chunk + suffix)
...
def _load_results_raw(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning("results.json corrupto (%s), intentando reparar: %s", filepath, e)
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                text = f.read()
        except OSError:
            return {}
        repaired = _repair_truncated_results_json(text)
        if repaired:
            logger.warning("results.json reparado parcialmente (%d claves)", len(repaired))
            return repaired
        bak = _results_backup_path(filepath)
...
def _atomic_write_results_json(filepath: str, data: Dict[str, Any]) -> None:
    os.makedirs(os.path.dirname(filepath) or ".", exist_ok=True)
    if os.path.isfile(filepath):
        try:
            shutil.copy2(filepath, _results_backup_path(filepath))
        except OSError:
            pass
    payload = dict(data)
    payload["_schema_version"] = _RESULTS_SCHEMA_VERSION
    payload["_updated_at"] = _results_now_iso()
    tmp = f"{filepath}.tmp"
...
def _merge_results_preserve(
    ...
    for key, value in incoming.items():
        if key.startswith("_"):
            out[key] = value
            continue
        if key not in _RESULTS_PRESERVE_KEYS:
            out[key] = value
            continue
        if key == "events":
        ...
        if value is None:
            continue
        if key in {"trades", "by_symbol", "metadata", "summary"} and not value:
            continue
        out[key] = value
    return out
...
def load_results_json(log_dir: str = "logs") -> Dict[str, Any]:
    filepath = _results_filepath(log_dir)
    with _RESULTS_JSON_LOCK:
        return _load_results_raw(filepath)
...
def save_results(results: dict, log_dir: str = "logs") -> None:
    ...
    filepath = _results_filepath(log_dir)
    with _RESULTS_JSON_LOCK:
        existing = _load_results_raw(filepath)
        merged = _merge_results_preserve(
            existing, results if isinstance(results, dict) else {}
        )
        _atomic_write_results_json(filepath, merged)
    logger.info("Resultados guardados en %s", filepath)


def patch_results(patch: Dict[str, Any], log_dir: str = "logs") -> Dict[str, Any]:
    log_dir = os.path.abspath(log_dir)
    filepath = _results_filepath(log_dir)
    with _RESULTS_JSON_LOCK:
        existing = _load_results_raw(filepath)
        merged = _merge_results_preserve(
            existing, patch if isinstance(patch, dict) else {}
        )
        _atomic_write_results_json(filepath, merged)
        return merged
...
def update_last_balance(
    ...
    body: Dict[str, Any] = {
        "last_balance": {
            **balance,
            "timestamp": balance.get("timestamp") or _results_now_iso(),
        },
    }
    meta_patch: Dict[str, Any] = {}
    te = balance.get("total_equity")
    ab = balance.get("available_balance")
    if te is not None:
        meta_patch["capital_actual"] = float(te)
        meta_patch["capital_final"] = float(te)
    if ab is not None:
        meta_patch["balance_available_balance"] = float(ab)
    if balance.get("account_type"):
        meta_patch["balance_account_type"] = balance.get("account_type")
    if balance.get("coin"):
        meta_patch["balance_coin"] = balance.get("coin")
    if meta_patch:
        body["metadata"] = {**prev_meta, **meta_patch}
    patch_results(body, log_dir=log_dir)


def validate_results_file(log_dir: str = "logs") -> Dict[str, Any]:
    filepath = _results_filepath(log_dir)
    with _RESULTS_JSON_LOCK:
        try:
            data = _load_results_raw(filepath)
            if not data:
                return {"ok": False, "error": "empty_or_missing", "path": filepath}
            json.dumps(data)
            trades = data.get("trades") or {}
            n_trades = (
                sum(len(v or []) for v in trades.values())
            ...
            return {
                ...
                "has_balance": bool(data.get("last_balance") or meta.get("capital_actual")),
...
_AUTO_GIT_WATCH_PATHS = ("src", "NerT_AI_PRO", "scripts", "config", "tests", "docs")


def _git_run(cmd: List[str], cwd: str) -> subprocess.CompletedProcess:
...
def _perform_auto_git_commit(reason: str = "runtime") -> None:
    ...
    if not status.stdout.strip():
        return
    for folder in _AUTO_GIT_WATCH_PATHS:
        p = os.path.join(root, folder)
        if os.path.exists(p):
            _git_run(["git", "add", "-A", "--", p], root)
    for extra in ("README.md", "requirements.txt", "pyproject.toml", ".gitignore"):
        ep = os.path.join(root, extra)
        if os.path.isfile(ep):
            _git_run(["git", "add", "--", ep], root)
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    msg = f"chore: auto-commit {reason}\n\n[auto-commit] {ts}"
    commit = _git_run(["git", "commit", "-m", msg], root)
    if commit.returncode != 0:
        return
    if os.getenv("AUTO_GIT_REMOTE", "").strip().lower() in {"1", "true", "yes"}:
        _git_run(["git", "push"], root)


def maybe_auto_git_commit(reason: str = "runtime") -> None:
    """Commit automático si AUTO_GIT_COMMIT=1 (no bloquea el motor)."""
    if os.getenv("AUTO_GIT_COMMIT", "").strip().lower() not in {"1", "true", "yes"}:
        return
    try:
        threading.Thread(
            target=_perform_auto_git_commit,
            args=(reason,),
            daemon=True,
        ).start()
    except Exception:
